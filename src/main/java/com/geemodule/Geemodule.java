...
public final class Geemodule {
    ...
    /**
     * Created the modules-loader without specifying the root-dmodules-path as a parameter. See {@link com.geemodule.Geemodule} to find
    ...
    /**
     * Creates the module-loader and calls its bootstrap method which initializes all modules and their dependencies. The modules-root-dir is
     * determined in the following order:
     * <ol>
     * <li>The modules-root-dir parameter passed into this method.</li>
     * <li>If parameter does not exist, try to retrieve it from the geemodule.properties. The geemodule.properties must either reside in the root of
     * the class-path or its location must be specified as a system property with the key 'geemodule.properties.path'. The latter has precedence.</li>
     * <li>If the properties file cannot be found or the entry 'root.module.path' is missing, attempt to load the value with
     * System.getProperty("root.module.path").</li>
     * <li>Should none of the above work an IllegalStateException is thrown and the modules cannot be initialized.</li>
     * </ol>
     * 
     * @param modulesRootDir
     *            Path to the root directory where all the modules reside.
     * @return ModuleLoader
     */
	...
	// Root modules path not passed as parameter. Lets check the geemodule.properties file.
	...
	// If it is still empty, try finding the root modules path in the system.
	if (Str.isEmpty(_rootModulesPath)) {
